...
module.exports = {
  ...
  usage:
    "@member be sure to tag the member directly after the command if you tag someone.",
  args: false,
  readOnly: false,
  ...
  execute: async (message, args, _redis, level) => {
    ...
    if (args.length === 0) {
      ({ member } = message);
      friendly = message.client.config.permLevels.find((l) => l.level === level)
        .name;
    }
    if (!member)
      return await message.channel.send(
        "This user can't be found. Try **!help info** and double check usage."
      );
