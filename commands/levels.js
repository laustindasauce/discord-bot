...
const embed = new Discord.MessageEmbed()
  ...
  .setTimestamp()
  .setFooter(
    "Try running `!my-level` to see your permission level.",
...
module.exports = {
  ...
  aliases: ["levels"],
  description:
    "Displays embedded message with all available permission levels in the server.",
  args: false,
  readOnly: false,
  ...
  execute(message, _args, _redis, _level) {
    for (let i = message.client.config.permLevels.length - 1; i >= 0; i--) {
      ...
      embed.addField(
        `\tLevel: ${thisLevel.level}\t{${thisLevel.name}}`,
        `\tDescription: ${thisLevel.description}`,
        false
      );
    }
    return message.author
      ...
      .then(() => {
        ...
        message.reply(
          "I've sent you a DM with the available permission levels for this server!"
        );
      })
      .catch((error) => {
        console.error(
          `Could not send permission levels DM to ${message.author.tag}.\n`,
          error
        );
