...
module.exports = {
   ...
   */
  execute(message) {
    // REmove the for loop and just create array of the message content and check if there
    ...
    if (profane_words.length > 0) {
      let data = [];
      message.delete();
      data.push("Profanity was found in your last message:");
      data.push(profane_words);
      ...
      redis.get(title).then(function (res) {
        ...
        let body =
          "You have now sent " +
          count +
          " messages marked as containing profanity.";
        data.push(body);
        if (3 - count == 1) {
          body =
            "If you send 1 more profane message you will be kicked from the server.";
        } else if (count > 2) {
          body =
            "You have been kicked from the server for sending too many messages containing profanity.";
        } else {
          count = 3 - count;
          body =
            "If you send " +
            count +
            " more profane messages you will be kicked from the server.";
        }
        data.push(body);
        body =
          "If you believe to have received this in error and the message you sent was clean, ";
        ...
        message.author
          ...
          .then(() => {
            if (res > 2) {
              ...
              message.member
                .kick(
                  "User has sent a message with profanity at least 3 times now."
                )
                .then(() => {
          ...
          .catch((error) => {
            console.error(
              `Could not send profanity warning DM to ${message.author.tag}.\n`,
              error
            );
            message.reply(
