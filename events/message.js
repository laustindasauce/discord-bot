...
module.exports = {
	...
	execute(message, Discord, client, redis, prefix, cooldowns) {
        ...
        if (command.channels && !command.channels.includes(message.channel.name)) {
            return message.reply(`I can only execute \`${command.name}\` in the channel(s) \`${command.channels}\`!`)
        }
        ...
        try {
            ...
            console.error(error);
            message.reply(`sorry, there was an error trying to execute \`${command.name}.\` \nPlease contact admin if issue persists.`);
        }
	},
	test() {
		return true;
